...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse signUp(SignUp login) {
        if(userRepo.findByEmail(login.email()).isPresent()) throw new BadCredentialsException("User Already Exist With UserName: "+login.email());
        User user=userMapper.toUserFromSignUp(login);
        user.setPassword(passwordEncoder.encode(login.password()));
        user=userRepo.save(user);
        return userMapper.toAuthResponseFromUserAccess("AccessToken",user);

    }

    @Override
    public AuthResponse login(Login loginReq) throws AccessDeniedException {
        User user=userRepo.findByEmail(loginReq.username()).orElseThrow(()->new BadCredentialsException("User Does Not Exist With UserName: "+loginReq.username()));
        if(!passwordEncoder.matches(loginReq.password(),user.getPassword())) throw new AccessDeniedException("Invalid Credentials !");
        return userMapper.toAuthResponseFromUserAccess("AccessToken",user);
